...
function generate(toPeterson) {
	...
	var long_general_tweet = stripTags(phrase) + " " + SITE_LINK;
	var short_general_tweet = stripTags(phrase);
	var peterson_tweet = JBP_HANDLE + " " + stripTags(phrase);

	if (toPeterson && peterson_tweet.length <= 280) {
		return peterson_tweet;
	} else if (!toPeterson && long_general_tweet.length <= 280) {
		return long_general_tweet;
	} else if (!toPeterson && short_general_tweet.length <= 280) {
		return short_general_tweet;
	} else {
		return null;
	}
...
function tweetGeneral() {
	...
	while (generatedPhrase == null) {
		generatedPhrase = generate(false);
	}
...
function tweetPeterson() {
	var generatedPhrase = generate(true);
	while (generatedPhrase == null) {
		generatedPhrase = generate(true);
	}
...
tweetGeneral();
setInterval(tweetGeneral, 300000); // General tweet every 50 minutes
